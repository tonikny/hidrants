...
export function SyncButton({ className, label }: SyncButtonProps) {
  ...
  const getTitle = () => {
    if (!stats || stats.total_pending === 0) {
      return "Sincronitzar amb OSM (Baixada)";
    }
  ...
  return (
    <div className="relative inline-block">
      ...
      {stats && stats.total_pending > 0 && !isSyncing && (
        <div
          className={`absolute -top-1.25 -right-1.25 text-white rounded-full w-4.5 h-4.5 text-[10px] flex items-center justify-center font-bold shadow-[0_1px_3px_rgba(0,0,0,0.3)] pointer-events-none z-10 ${stats.PENDING_CREATE > 0 ? "bg-[#f1c40f]" : "bg-[#3498db]"}`}
        >
          {stats.total_pending}
        </div>
      )}
