        ...
        <label className="text-[0.8rem] italic flex flex-col gap-[4px]">
          ...
          >
            {TIPUS_INCIDENCIA.map((t) => (
              <option key={t.value} value={t.value}>{t.icon ? `${t.icon} ${t.label}` : t.label}</option>
            ))}
          </select>
        </label>

        <div className="flex gap-2">
          <label className="flex-1 text-[0.8rem] italic flex flex-col gap-[4px]">
            Estat:
            <select
              value={newStatus}
          ...
          <label className="flex-1 text-[0.8rem] italic flex flex-col gap-[4px]">
            ...
            >
              {PRIORITATS_INCIDENCIA.map((prioritat) => (
                <option key={prioritat.value} value={prioritat.value}>{prioritat.icon ? `${prioritat.icon} ${prioritat.label}` : prioritat.label}</option>
              ))}
            </select>
          </label>
        </div>
        ...
        <label className="text-[0.8rem] italic flex flex-col gap-[4px]">
          ...
          >
            {PRECISIONS_INCIDENCIA.map((precisio) => (
              <option key={precisio.value} value={precisio.value}>{precisio.icon ? `${precisio.icon} ${precisio.label}` : precisio.label}</option>
            ))}
          </select>
        </label>
        ...
        <label className="text-[0.8rem] italic flex flex-col gap-[4px]">
          ...
          >
            {VISIBILITATS_INCIDENCIA.map((visibilitat) => (
              <option key={visibilitat.value} value={visibilitat.value}>{visibilitat.icon ? `${visibilitat.icon} ${visibilitat.label}` : visibilitat.label}</option>
            ))}
          </select>
        </label>
